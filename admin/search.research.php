...
$oPage->setTitle(Translations::get('website_name') . ' | ' . Translations::get('admin_page_searchresearch_title'));
...
function createContent() {
	...
	// title
	$ret = '<h2>' . Translations::get('admin_page_searchresearch_title') . ' ' . Misc::createYearSwitcher($year,Settings::get('first_year'), date("Y")) . '</h2>';
	...
	$oView->set_view( array(
		'query' => "
SELECT visitors.id, visitor_misc.firstname, visitor_misc.lastname, visitor_misc.subject
FROM `visitors`
	INNER JOIN `visitor_misc` ON `visitors`.id=`visitor_misc`.visitor_id
WHERE `visitors`.is_deleted=0
	AND `visitor_misc`.is_deleted=0
	AND `visitor_misc`.subject IS NOT NULL
	AND `visitor_misc`.subject <> ''
	...
	$oView->add_field( new class_field_date ( array(
		'fieldname' => 'subject'
		, 'fieldlabel' => Translations::get('fld_subject')
		, 'view_max_length' => 55
		, 'viewfilter' => array(
			'filter' => array (
				array (
