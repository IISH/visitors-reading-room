...
$oPage->setTitle(Translations::get('website_name') . ' | ' . Translations::get('admin_page_statistics_emaildomain_short_title'));
...
function createContent() {
	...
	// title
	$ret = '<h2>' . Translations::get('admin_page_statistics_emaildomain_short_title') . ' ' . Misc::createYearSwitcher($year,Settings::get('first_year'), date("Y")) . '</h2>';
...
WHERE email IS NOT NULL AND email <> ''
	AND substring(email, LOCATE('@', email)+1) NOT IN ( " . Misc::getDomainNameExclusionsForSqlQuery() . " )
